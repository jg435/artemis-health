...
import { ouraAPI, type OuraTokens } from './oura';
...
export class OuraAuthService {
  ...
  async saveUserTokens(
    ...
    try {
      console.log('Saving Oura tokens for user:', userId);
      ...
      // Deactivate any existing Oura connections for this user
      await this.supabase
        .from('user_integrations')
      ...
      if (error) {
        console.error('Error saving Oura tokens:', error);
        console.error('Full error details:', JSON.stringify(error, null, 2));
        throw new Error(`Failed to save Oura connection: ${error.message}`);
      }
      
      console.log('Oura tokens saved successfully to database');
    } catch (error) {
  ...
  async getValidTokenForUser(userId: string): Promise<string | null> {
    try {
      // Get the active Oura integration for this user
      ...
      if (now.getTime() >= (expirationTime.getTime() - bufferTime)) {
        ...
        try {
          ...
          const newTokens = await ouraAPI.refreshToken(integration.refresh_token);
